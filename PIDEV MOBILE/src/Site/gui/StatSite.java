 ...
 * @author wajih
 */
public class StatSite extends BaseForm{
        Form current;

        public StatSite(Resources res) {
        super("Stat Stite", BoxLayout.y());
...
protected CategorySeries buildCategoryDataset(String title, double[] values) {
    ...
    */
    ...
    int nbmoy=0;
    int nbinf=0;
             List<Sitetouristique> lista = ServiceSitetouristique.getInstance().getAllSitetouristiques();;
             for (Sitetouristique fi : lista) {
                 if(fi.getPrix()>200){
                     nbsupp++;
                 }
                 else if(fi.getPrix()>100){
                     nbmoy++;
                 }
                 else
                 {
            ...
            series.add("Elevée" ,nbsupp);
            series.add("Moyenne" ,nbmoy);
            series.add("Minimum" ,nbinf);
